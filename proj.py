...
class Huffman:
  ...
  def compressed_path(self):
    print "Path of compressed file:",os.path.abspath("cmpr")
    return os.path.abspath("cmpr")
  ...
  def padding(self,encoded):
    ...
    #print "Encoded msg after padding:",encoded
    return encoded
  ...
  def writeFile(self,text):
    ...
    for i in text:
        f.write(i)
    #print "Path of decompressed file:",os.path.abspath("Decompressed")
    f.close()
  ...
  def makeTree(self,freq_of_char) :
    while len(freq_of_char) > 1 :
        smallest = (freq_of_char[0],freq_of_char[1])                  # get the 2 to combine
        nodes  = freq_of_char[2:]                          # all the others
        totalNodeVal = smallest[0][0] + smallest[1][0]     # the branch points freq
        freq_of_char   = nodes + [(totalNodeVal,smallest)]     # add branch point to the end
        self.sort(freq_of_char)                              # sort it into place
